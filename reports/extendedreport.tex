...
It remains to be seen how applicable the optimal $V_{max}$ values are to bacterial cultures. Optimal $V_{max}$ values for some reactions are at 0.1 $V_{max}$, at the limit, which could indicate the activity of the corresponding enzyme should be minimised
Otherwise, the issue is with translating optimal $V_{max}$ values to enzyme concentrations and appropriate gene expression control.
%[Move this to `summary of discussion' part I plan to write at the end of the discussion]
