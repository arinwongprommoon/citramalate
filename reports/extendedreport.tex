...
% Boundaries for FBA:
The results suggest that kinetic model simulations are useful in informing the flux bounds for FBA on the stoichiometric model, and differential evolution is a useful strategy to establish such flux bounds. Specifically, the 41-dimension bounds found by differential evolution were mostly wider and bounds found by individually varying $V_{max}$ values in the kinetic model. An alternative of using sets of six reactions that have the greatest bearing on each reaction in the kinetic model was investigaed; it turned out that these six-reaction sets for each reaction in the kinetic model are similar sets of reaction that have high flux control coefficients and that the resulting flux boundaries were no better than previously found.

However, it remains to be seen how realistic \emph{in vivo} are the highest and lowest possible flux values are.
...
FBA suggested a fork between the growth and citramalate synthesis reactions, as evidenced by how optimising one excludes flux through the other. Alternatively, this phenomenon could be explained by how the fluxes of these reactions are not bounded, but this explanation is not realistic \emph{in vivo}. This observation may imply that optimising citramalate productivity in culture must take into account growth in addition to changing enzyme concentrations. Therefore, any FBA does not optimise for growth and citramalate synthesis simultaneously, and this issue may be addressed using guarded flexible nets \citep{julvez_handling_2018}.
