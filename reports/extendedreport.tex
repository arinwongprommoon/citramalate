...
% [This might be more appropriate in the discussion; it is an important part of the project and it does relate the two models, but it seems out of place in an otherwise `expository' introduction.]
...
The one-reaction list identified CITRA\_SYN, GLT, LPD, GROWTH, ATP\_MAINTENANCE, GDH, ATP\_SYN, ACEA, PYK, and ZWF as the reactions that had the greatest effects on citramalate productivity. %[Move this to discussion or something] \citet{millard_metabolic_2017} listed CYTBO, ZWF, GDH, GLT, and GROWTH as the reactions found to have the largest shares of flux control. They also exert the strongest controls on concentration. These reactions are followed by LPD, ATP\_MAINTENANCE, ACEA, ATP\_SYN, and PYK.

Enzymes that have the largest shares of flux control as noted by \citet{millard_metabolic_2017} showed the largest effects on citramalate productivity. However, CYTBO had less effect on productivity as would be expected by this explanation. Although PYK did not exert much control over fluxes as would be expected from its large flux control coefficient, it had a large effect on citramalate productivity. %[Move this to discussion] Possibly it was because the citramalate reaction used up pyruvate and because PYK is a control point in glycolysis.
...
% Significance of this?? -- comment that it has the potential to complicate finding Vmax combinations to optimise productivity, but then was sort of resolved by DE at the end --> DISCUSSION
...
%[Move this sentence to the discussion] This suggested that not all reactions in a network have to be included in differential evolution to produce valid results. The conclusion is beneficial as it reduces computation time needed to optimise citramalate productivity.
...
%% Just fucking put my final map in
...
%%[Probably move this to discussion]
%All these mapping procedures suggest a potention general strategy or algorithm in reconciling two types of models that exhibit differences.
...
% [This sentence can be moved to the discussion]
...
% 0.712 citramalate yield compared to 0.2 of ConBioChem and 0.77 of Wu and Etiman:
% This shows that the model(s) predict a yield that is in the range of real life, giving legitimacy points to our model(s)
% Further discussion of Wu and Etiman (as per my notes) and link them to the one-reaction findings in my project

% Validity of the one-reaction list:
% 0.5-2.0 range is arbitrary, but we need some sort of range. Using FCC list is not appropriate as we want to prioritise citramalate production. This is supported by the differences between the one-reaction list and the FCC list. In any case, DE demonstrates the utility of the one-reaction list, and DE on 41 reactions shows that you can do away with it if you like.

% Explaining differences between one-reaction list and FCC list: exchange reactions, reactions that directly concern AcCoA, etc.

% Inflection points: can complicate heatmaps and DE, but it is later shown that it really doesn't
...
% Differential evolution is useful:
% - Agrees with heatmaps
% - Produces greater productivity
% Other genetic algorithms? Why use differential evolution in particular? Please address this.

% Limitation of differential evolution: curse of dimensionality, but both optimisation of citramalate production and glycolytic enzyme exercises demonstrate that there isn't much need to add reactions down the list (thus also confirming that the one-reaction plots are useful)

% Finding optimal parameters for differential evolution:
% Rather than using the values in the table, dithering or black-box optimisation can be used (and yes, I have tried). However, the memory leak gets in the way of this, plus black-box optimisation. Arguably a thorough investigation of all methods to optimise differential evolution parameters may not be needed or may be useless -- these parameters depend wildly on the number of reactions or the functions, so it would be difficult anyway. Perhaps a general guideline may be more useful, but this is beyond the scope of this project -- let the people who study differential evolution in its own right take care of this.

% How applicable are the optimal $V_{max}$ values to real life? (this can be right at the end of this section):
% Some of them are 0.1 $V_{max}$, at the limit. Maybe this just indicates that enzyme activity should be minimised?
% Otherwise, the issue is with translating optimal %V_{max}% to enzyme concentration to gene expression control. With how wet-lab biology works it's probably going to be practically impossible to get exactly the $V_{max}$ stated (cloning, noise in gene expression, things being poorly characterised, etc) though.

% Investigating glucose uptake:
% (see evernote -- link between decoupling and glucose uptake from environment, how apply to lund)

\section{Stoichiometric model}
\label{sec:discussion-stoichiometric}

%% TOPICS TO DISCUSS, IN BRIEF

% Unit conversion (Jorge proposed a different number apart from 6.372)

% Boundaries for FBA:
% How realistic \emph{in vivo} are the highest and lowest possible flux values?
% This shows how useful differential evolution is, again

% Mapping:
% Suggests a general strategy for mapping and reconciling different structures in sub-networks, which can be streamlined later. Really, really useful thing to come out of this project as this may be applied to other models.
% How valid is my strategy of reconciling different structures in sub-networks though?

% Implications for fork between fluxes of growth and citramalate synthesis??

% Analysing flux bounds, what are the implications?:
% - Upper bound = lower bound -> flux fixed to one possible value, represents regulation? There are quite a lot of reactions (912) that are like this, which reactions are they? Why do you think they are this way?
% - `Fake reversible' -> does this mean a different way of looking at the reactions? Some of these have been established as reversible in studies on thermodynamics. Does this suggest a flaw in the model or a flaw in our reasoning to determine reactions that are not truly reversible?
% - Why are ATPS4rpp and GLCtex forced to be active? What do they do?
% - Say that is `not surprising' that a lot of transport reactions are forced to be active.

% FVA and pFBA:
% Agree with FBA results. This means that these methods are concordant with FBA. Comment on FVA not being able to return results when the biologically relevant loopless condition is used (explain what the loopless condition is). Perhaps in the long run we can stick with FBA because it doesn't have these problems and because it takes less compuatational time.

% Lund:
% How does one get a reliable value for OD600?
% I think I made some more notes here
% This is the only bit where NOTHING is resolved
