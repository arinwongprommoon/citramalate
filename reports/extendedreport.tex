...
Findings may then be generalised to cases not specific to \emph{E. coli} or citramalate production
...
I investigated the kinetic model to determine the combination of $V_{max}$ values that would optimise citramalate productivity. This parameter was chosen as it can be easily tested \emph{in vivo} based on how $V_{max}$ is proportional to enzyme concentration. I started from varying $V_{max}$ values one and two reactions at a time to establish patterns. Then, I used differential evolution to vary multiple $V_{max}$ values simultaneously, employing the genetic algorithm to narrow down the solution space.
...
% [I really don't think the inflection points are important -- it only seemed to affect the heatmaps, and even then I ditched the heatmaps in favour of DE later.]
...
The results are only valid if the system has reached steady state, because the focus is on continuous cultures and because the project aims to enrich a stoichiometric model. Steady state is achieved if at the end of simulation time:
...
% %% [This section can easily be killed -- the only thing I took from it is that addding enzymes lower on the one-reaction list doesn't change much. It doesn't relate to anywhere else in the report, and frankly, it's kind of obvious.]
...
%% [This section can be killed. The aim is a bit misguided; of course there is decoupling between glucose exchange and glucose uptake at some point (i.e. saturation or something) -- it makes biological sense. Additionally, this section can be siphoned off to another paper. It's a bit out of place, and results from this aren't used anywhere else.]
...
Results suggested that the model predicted decoupling between glucose exchange and glucose uptake only after a certain saturation point.
...
Before June 2018, Anargyros Chaliotis produced a mapping table which mapped reactions in the kinetic model to reactions in the stoichiometric model. %Flux bounds were also included, but the method by which the bounds were generated had not been documented and no specific constraints had been used.
...
However, when the loopless condition was not applied and FVA was set to optimise growth, numerical results were returned.
...
% Boundaries for FBA:
The results suggest that kinetic model simulations are useful in informing the flux bounds for FBA on the stoichiometric model, and differential evolution is a useful strategy to establish such flux bounds. Specifically, the 41-dimension bounds found by differential evolution were mostly wider and bounds found by individually varying $V_{max}$ values in the kinetic model.
I also investigated using sets of six reactions that have the greatest bearing on each kinetic model reaction in differential evolution.  It turned out that these six-reaction sets consist of similar reaction with high flux control coefficients, and the resulting flux boundaries were no better than previously found.
In any case, it remains to be seen how realistic \emph{in vivo} are the highest and lowest possible flux values are.

Additionally, mapping reactions in the kinetic model to the stoichiometric model suggested a general strategy for mapping reactions that have one-to-one correspondence, and reconciling different sub-network structures for reactions that do not. Such a strategy may be applied to models of other metabolic networks. However, so far the mapping does not include a strategy to map the biomass production reaction of the two models. Devising such a strategy can lead to determining a strictly positive flux bound for the biomass reaction of the stoichiometric model and aid in optimising growth and citramalate production simultaneously.

\subsection{Flux balance analysis}
\label{ssec:discussion-stoichiometric-fba}

FBA suggested a fork between the growth and citramalate synthesis reactions, as evidenced by how optimising one excludes flux through the other. Alternatively, this phenomenon could be explained by how the fluxes of these reactions are not bounded, but this explanation is not realistic \emph{in vivo}. Therefore, FBA does not optimise for growth and citramalate synthesis simultaneously, and this issue may be addressed using guarded flexible nets \citep{julvez_handling_2018}.
...
There are challenges to the validity of using a Monod model to relate $C_{glc}^{feed}$ to $C_{x}$ as in equation~\ref{eqn:taymaznikerel10_lreg}.  \citet{taymaz-nikerel_genome-derived_2010} suggest that there is a linear correlation between $D$ and $C_{x}$, with $r$ = 0.868, but for simplicity this factor was excluded as we only concern $D$ = 0.1.  Only three data points were used to construct equation~\ref{eqn:taymaznikerel10_lreg}, but the lack of precision can be justified by our aim of finding a flux boundary rather than modelling the relations.  Additionally, \citet{kovarova-kovar_growth_1998} challenge Monod's model, but confirm that Monod's proposal and its slight modifications are sufficient to describe the growth of pure cultures with single growth-controlling substrates under ideal conditions. This assertion was supported by data for growth of \emph{E. coli} with glucose.

\citet{schulze_relationship_1964} present:
...
where $Y$ is the yield factor, the unit weight of cells formed per unit weight of substrate. This equation implies that a linear regression model for $C_{x}$ with respect to $C_{glucose}^{feed}$ must have a negative vertical intercept, contrary to equation~\ref{eqn:taymaznikerel10_lreg}.  However, cell lysis may add a constant biomass concentration to the right-hand-side of equation~\ref{eqn:monodmodel} so that the intercept becomes positive.  \citet{taymaz-nikerel_genome-derived_2010} suggest that such cell lysis is significant.
...
This is the future direction of the project (as of 10 September 2019):

\begin{enumerate}
  \item Devise a strategy to map the biomass growth reactions of the kinetic and stoichiometric models. This is so that a way to force positive biomass growth can be developed. Compute flux bounds accordingly.
\item Use guarded flexible nets \citep{julvez_handling_2018} to optimise citramalate production and biomass growth simultaneously in the stoichiometric model. Account for uncertainties. Take into account Lund medium accordingly.
\end{enumerate}
...
Further work was done 27 August 2018 -- 3 October 2018 and 12 July 2019 -- 10 September 2019.
