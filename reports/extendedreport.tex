...
%[Move the following to `summary of discussion' part I plan to write at the end of the discussion]
% With how wet-lab biology works it's probably going to be practically impossible to get exactly the $V_{max}$ stated (cloning, noise in gene expression, things being poorly characterised, etc) though.
...
% Boundaries for FBA:
The results suggest that kinetic model simulations are useful in informing the flux bounds for FBA on the stoichiometric model, and the differential evolution is a useful strategy to establish such flux bounds. Specifically, the 41-dimension bounds found by differential evolution were mostly wider and bounds found by individually varying $V_{max}$ values in the kinetic model.
%[Move to `summary of discussion'] How realistic \emph{in vivo} are the highest and lowest possible flux values?
...
% Implications for fork between fluxes of growth and citramalate synthesis?? --> link/mention this in the end section where i tie this to real-life applications
