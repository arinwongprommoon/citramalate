...
def check(value, message):
    if value == None:
        raise SystemExit('LibSBML returned a null value try to ' + message + '.')
...
def create_model():
    ...
    check(k.setConstant(True), 'set paramater k "constant"')
    check(k.setValue(1), 'set parameter k value')
    check(k.setUnits('per_second'), 'set parametr k units')
    ...
    check(species_ref1, 'create product')
    check(species_ref1.setSpecies('s2'), 'assign product species')
    check(species_ref1.setConstant(True), 'set "constant" on species ref 2')
